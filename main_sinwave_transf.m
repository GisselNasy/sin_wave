...
%  INITIALIZATION
w0 = readmatrix('csv_w0_tr3_nn_sinwave_transformer.csv');
...
n_weight_nn_elm = sw_nn_elm(6,2);

%  INITIALIZATION
w0_nn_elm = readmatrix('csv_w0_tr3_nn_elm_sinwave_transformer.csv');
...
options = optimoptions('fminunc', ...
    ...
    'MaxIterations', max_iter, ...
    'MaxFunctionEvaluations', 1e10, ...
    'OutputFcn', @(w,optimValues,state) outfun(w,optimValues,state,x,u0,nn_elm,sw_nn_elm));

tic;
[w_stage1,~,~,out1] = fminunc(@(w) problem_fit2_nn_elm(w,x,u0,nn_elm,sw_nn_elm), w0_nn_elm, options);
t_stage1 = toc;
...
W1 = reshape(w_stage1(sw_nn_elm(1,1):sw_nn_elm(1,2)), nn_elm(1), nn_elm(2));
B1 = reshape(w_stage1(sw_nn_elm(2,1):sw_nn_elm(2,2)), 1, nn_elm(2));
H1 = g(x*W1 + B1);
H1 = [x H1];
w2 = w_stage1(sw_nn_elm(3,1):sw_nn_elm(6,2));

sw_edit_nn_elm = sw_nn_elm - sw_nn_elm(2,2);
outfun_phase2_handle = @(w,optimValues,state,varargin) outfun_phase2_local(w,optimValues,state,H1,u0,nn_elm,sw_edit_nn_elm,n);
...
[w2,~,~,out2] = fminunc(@problem_fit3_nn_elm, w2, options, H1, u0, nn_elm, sw_edit_nn_elm);
...
w_stage2 = [w_stage1(sw_nn_elm(1,1):sw_nn_elm(2,2)) w2];
% W1 = reshape(w_stage2(sw(1,1):sw(1,2)), nn(1), nn(2));
% B1 = reshape(w_stage2(sw(2,1):sw(2,2)), 1, nn(2));
W2 = reshape(w_stage2(sw_nn_elm(3,1):sw_nn_elm(3,2)), nn_elm(2)+1, nn_elm(3));
B2 = reshape(w_stage2(sw_nn_elm(4,1):sw_nn_elm(4,2)), 1, nn_elm(3));
...
w_nn_elm = [w_stage2(1:sw_nn_elm(4,2)) w3'];
t_elm = toc;

%  Evaluation
u_pred_nn_elm = NN(w_nn_elm, x, nn_elm, sw_nn_elm);
mse_nn_elm = (2/size(x,1))*problem_fit2_nn_elm(w_nn_elm,x,u0,nn_elm,sw_nn_elm);
...
u_pred_elm = NN(w_elm,x,nn_elm,sw_nn_elm);
mse_elm = (2/size(x,1))*problem_fit2_nn_elm(w_elm,x,u0,nn_elm,sw_nn_elm);
...
%% helper function
function stop = outfun(w, optimValues, state, x, u0, nn_elm, sw_nn_elm, varargin) % Tambahkan varargin
    stop = false;
    global MSE_hist time_hist
    switch state
        case 'init'
            % Ambil hanya output pertama (L) dari problem_fit
            [L, ~] = problem_fit2_nn_elm(w, x, u0, nn_elm, sw_nn_elm);
...
function stop = outfun_phase2_local(w,optimValues,state,H1,u0,nn_elm,sw_nn_elm,n,varargin)
stop = false;
...
switch state
    case 'init'
        MSE_hist(end+1) = (2/n)*problem_fit3_nn_elm(w,H1,u0,nn_elm,sw_nn_elm);
        time_hist(end+1) = toc;
